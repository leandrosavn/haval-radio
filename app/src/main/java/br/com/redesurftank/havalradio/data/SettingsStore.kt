...
object SettingsStore {
    ...
    private const val KEY_DOCK_RESERVE = "reserve_dock_bar"
    private const val DEFAULT_DOCK_RESERVE = true

    /** Altura da barra inferior do Haval Dock (mesmo valor do OverlayService de lá). */
    const val DOCK_BAR_DP = 84
    ...
    /** Reserva espaço no rodapé p/ a barra do Haval Dock não cobrir os controles. */
    val reserveDockBar = mutableStateOf(DEFAULT_DOCK_RESERVE)

    fun init(context: Context) {
        appCtx = context.applicationContext
        launchOnBoot.value = prefs(appCtx).getBoolean(KEY_BOOT, DEFAULT_BOOT)
        reserveDockBar.value = prefs(appCtx).getBoolean(KEY_DOCK_RESERVE, DEFAULT_DOCK_RESERVE)
    }
    ...
    fun setReserveDockBar(value: Boolean) {
        reserveDockBar.value = value
        prefs(appCtx).edit().putBoolean(KEY_DOCK_RESERVE, value).apply()
    }
